...
parameters {
  real<lower=0, upper = 1> a1;       // accuracy for Class 1
  real<lower=0, upper = 1> a0;       // accuracy for Class 0
...
model {
  ...
  for(i in 1:N) {  // for each subject rated
    target += n[i] * log_sum_exp(log(t)   + binomial_lpmf(N_c[i] | N_r[i], p_true(a1,p)),
                          log(1-t) + binomial_lpmf(N_c[i] | N_r[i], p_false(a0,p)));
  }
}
