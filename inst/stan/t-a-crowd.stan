...
//
//   Stan model specification for free parameters t and a, with
//   p determined by counts, as a way to introduce dependency
//
...
parameters {
  ...
  real<lower=0, upper = 1> p;        // true class 1 rate
}
...
model {
  ...
  for(i in 1:N) {  // for each subject rated
    real p_crowd = p* (N_c[i] + .5) / (N_r[i] + 1); // wisdom of the crowds?
    target += n[i] * log_sum_exp( log(t)   + binomial_lpmf(N_c[i] | N_r[i], p_true(a,p_crowd)),
                            log(1-t) + binomial_lpmf(N_c[i] | N_r[i], p_false(a,p_crowd)));
  }
}
